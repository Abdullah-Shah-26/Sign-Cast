...
def main():
    ...
    if not python_path.exists():
        ...
        print("   Expected: apps/backend/.venv (recommended) or apps/backend/py311_venv")
        sys.exit(1)
...
'''
    ...
    # Report the output executable location
    dist_dir = backend_dir / "dist"

    # Determine the built executable
    if platform.system() == "Windows":
        built_exe = dist_dir / "backend.exe"
    else:
        built_exe = dist_dir / "backend"
    
    if built_exe.exists():
        print(f"✅ Backend executable built at: {built_exe}")
    else:
        print(f"❌ Backend executable not found at {built_exe}")
        sys.exit(1)
