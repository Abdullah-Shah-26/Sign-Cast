...
/// <reference path="../types/pose-viewer.d.ts" />
...
const PoseViewer: React.FC<PoseViewerProps> = ({
  ...
  useEffect(() => {
    ...
    if (poseFile) {
      try {
        const blob =
          poseFile instanceof Blob
            ? poseFile
            : new Blob([poseFile], { type: "application/octet-stream" });
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center">
          ...
          <p className="text-sm text-danger-600 font-medium mb-2">{error}</p>
    ...
    <div className="h-full flex flex-col max-h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between mb-4 px-2 flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          <div
            className={`w-2 h-2 rounded-full ${isTranslating ? "bg-warning-500 animate-pulse" : isPlaying ? "bg-success-500 animate-pulse" : "bg-success-500"}`}
          ></div>
      ...
      {/* Animation Container */}
      <div className="flex-1 relative overflow-hidden rounded-lg bg-theme-secondary border border-theme-primary min-h-0">
        <div ref={containerRef} className="pose-viewer-container h-full w-full">
          <pose-viewer
            ...
            className="w-full h-full max-h-full pose-viewer-dark-mode"
          />
        </div>
        ...
        <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex items-center gap-2 bg-black/50 backdrop-blur-sm rounded-full px-3 py-1">
          <button
            onClick={() => {
              const poseViewer = containerRef.current?.querySelector(
                "pose-viewer",
              ) as any;
              if (poseViewer) {
                if (isPlaying) {
